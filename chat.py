...
root.iconbitmap('ai_lt.ico') # from https://tkinter.com/images/ai_lt.ico
    ...
    if chat_entry.get():
        ...
        try:
            if os.path.isfile(filename):
                ...
                my_text.insert(END, (response['choices'][0]['text']).strip())  # strip will strip out any before/after line breaks
                my_text.insert(END, '\n\n')
...
def clear():
    ...
    my_text.delete(1.0, END) # 1.0 is the first position in the tkinter box -> delete from top all the way to bottom (END)
    # Clear the Query Entry Widget
    chat_entry.delete(0, END) # entry boxes start from 0 instead of 1.0 (for some weird reason)
